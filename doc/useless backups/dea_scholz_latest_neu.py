...
def convert(r):
        ...
        for i in range(0,len(right)):
            ...
            if right[i] in T:
                ...
                if final == 0:
                    ...
                    right = left + "_final"
        statename = "q_" + str(len(statelist))
        outcomes = []
        outcomes.append(right)
        alphabet = []
        alphabet.append(item)
        #print(str(outcomes))
        newstate = State(right, outcomes, alphabet, 100, 100, final)
        #print(left)
        #print(newstate.getoutcomes())
        statelist.append(newstate)
        #zu nea hinzufügen, wenn es noch keinen mit gleichem name gibt
        if not newstate.name() in neanamelist:
            nea.append(newstate)
            neanamelist.append(newstate.name())
        #print(right + ": " + str(final))
    ...
    #create dea
    #wenn keine "doppelnutzung": direkt hinzufügen
    """for state in nea:
        ambig = 0
        itemlist = []
        ...
        if ambig == 0:
            dea.append(state)
            print(state.name() + " ist eindeutig")
            print(state.getalphabet())
        else:
            print(state.name() + " ist nicht eindeutig")
            print(state.getalphabet())
    """
    """
    for state in nea:
        ...
        while ambig > 0:
            ambig = 0
            #prüfen, ob zustand eindeutig ist
            for item in state.getalphabet():
                if item in itemlist:
                    ambig = 1
                itemlist.append(item)
    ...
    for state in nea:
        ...
        for i in range(0,len(state.getalphabet())):
            for j in range(0,len(state.getalphabet())):
                if state.getalphabet()[i] == state.getalphabet()[j]:
                    if i < j:
                        #if not(state.getalphabet()[i] == state.getalphabet()[j]):
                        if 1 == 1:
                            ambig = 1
                            completeamb = 1
                            print("using " + state.getalphabet()[i] + " results in: ")
                            print("state[i]: " + state.getoutcomes()[i])
                            print("state[j]: " + state.getoutcomes()[j])
                            for zst in nea:
                                ...
                                if state.getoutcomes()[j] == zst.name():
                                    statej = zst
                            deastatelist.append(melt(statei,statej))
                            #melt(statei,statej)   
                            print("melt one state from " + statei.name() + " and " + statej.name())
        if ambig == 0:
            deastatelist.append(state)
            #print(state.name() + " ist eindeutig")
            #print(state.getalphabet())
            ...
            #print(state.getalphabet())
    if completeamb == 1:
        #print("noch nicht eindeutig, neustart")
        deastatelist = neatodea(deastatelist)
    else:
        #print("alles eindeutig, erfolg")
        #print("statelist (= dea): " + str(deastatelist))
        return(deastatelist)

def melt(statex,statey):
    #print("melting initialized")
    name = statex.name() + statey.name()
    outcomes = []
    alphabet = []
    global T
    global deastatelist
    ...
    #t: alphabetelemente
    for item in T:
        for x in range(0,len(statex.getalphabet())):
            for y in range(0,len(statey.getalphabet())):
                if statex.getalphabet()[x] == statey.getalphabet()[y]:
                    alphabet.append(statex.getalphabet()[x])                   
                    outcomes.append(statex.getoutcomes()[x]+statey.getoutcomes()[y])
                    #print("appended")
                    #listall = nea + deastatelist
                    for statexyz in (nea+deastatelist):
                        if statexyz.name() == statex.getalphabet()[x]:
                            outx = statexyz
                        if statexyz.name() == statey.getalphabet()[y]:
                            outy = statexyz
                    #deastatelist.append(melt(outx,outy))
                    #melt(outx,outy)
    if (statex.final() + statey.final()) == 0:
        final = 0
    else:
        final = 1
    resultstate = State(name, outcomes, alphabet, 100, 100, final)
    #deastatelist.append(resultstate)
    return(State(name, outcomes, alphabet, 100, 100, final))
