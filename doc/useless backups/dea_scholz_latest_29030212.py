...
def melt(name):
    ...
    meltlistdone.append(name)
    if len(name) > 1:
        print("meltlist: " + str(meltlist))
        print("new item used")
        print("name :" + name)
        namex = name[:name.index("|")]
        namey = name[name.index("|")+1:]
        for state in (nea + deastatelist):
            if state.name() == namex:
                statex = state
            if state.name() == namey:
                statey = state
        statename = namex+namey
        outcomes = []
        alphabet = []
        ...
        for item in T:
            ...
            outcomeparts = []
            outcomename = ""
            for n in range(0,len(statex.getalphabet())):
                if statex.getalphabet()[n] == item:
                    itemused = 1
                    outcomename = outcomename + statex.getoutcomes()[n]
            for n in range(0,len(statey.getalphabet())):
                if statey.getalphabet()[n] == item:
                    itemused = 1
                    if not statey.getoutcomes()[n] in outcomeparts:
                        outcomeparts.append(statey.getoutcomes()[n])
            for outcomepart in outcomeparts:
                if len(outcomename) == 0:
                ...
                if statenew == 0:
                    ...
                    if 1 == 1:
                        if (outcomename not in meltlist):
                            meltlist.append(outcomename)
                            print("item added: " + outcomename)
            if itemused == 1:
                alphabet.append(item)
        #print(alphabet)
        #print(outcomes)        
        deastatelist.append(State(statename, outcomes, alphabet, 100, 100, final))
