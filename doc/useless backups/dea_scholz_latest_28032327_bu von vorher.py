...
def neatodea(nea):
            ...
            for j in range(0,len(state.getalphabet())):
                if state.getalphabet()[i] == state.getalphabet()[j]:
                    if i < j:
                        ambig = 1
                        completeamb = 1
                        #statelist.append(melt(state.getoutcomes()[i],state.getoutcomes()[j]))
                        print("melt one state from " + state.getoutcomes()[i] + " and " + state.getoutcomes()[j])
        if ambig == 0:
    ...
    if completeamb == 1:
        print(")
        statelist = neatodea(statelist)
    else:
        print("alles eindeutig")
        return(statelist)
